...
export function useChat(userId: string) {
  ...
  useEffect(() => {
    const unsubs = [
      chatWs.on('chat.message', (data: { message?: Message } & Record<string, unknown>) => {
      ...
      chatWs.on('chat.send.ack', (data: { client_request_id?: string; message_id?: string; created_at?: string }) => {
      ...
      chatWs.on('chat.typing', (data: { user_id?: string; channel_id?: string; is_typing?: boolean }) => {
        if (data.user_id === userId || data.channel_id !== activeChannelId) return
        setTypingUsers(prev => {
      ...
      chatWs.on('chat.react', (data: { message_id?: string; user_id?: string; emoji?: string; remove?: boolean }) => {
        setMessages(prev => prev.map(m => {
          ...
          } else if (!users.includes(data.user_id!)) users.push(data.user_id!)
      ...
      chatWs.on('chat.read', (data: { user_id?: string; message_id?: string }) => {
      ...
      chatWs.on('chat.edit', (data: { channel_id?: string; message_id?: string; content?: string; edited_at?: string }) => {
        if (data.channel_id !== activeChannelId) return
        setMessages(prev => prev.map(m =>
          m.message_id === data.message_id ? { ...m, content: data.content!, edited_at: data.edited_at } : m
        ))
      }),

      chatWs.on('chat.delete', (data: { channel_id?: string; message_id?: string }) => {
        if (data.channel_id !== activeChannelId) return
        setMessages(prev => prev.map(m =>
          m.message_id === data.message_id ? { ...m, content: '', deleted_at: (data as any).deleted_at, deleted_by: (data as any).deleted_by, reactions: undefined, mentions: undefined, attachments: undefined } : m
        ))
        setPinnedMessages(prev => prev.filter(m => m.message_id !== data.message_id))
      }),

      chatWs.on('chat.pin', (data: { channel_id?: string; message_id?: string; unpin?: boolean; pinned_at?: string; pinned_by?: string }) => {
        if (data.channel_id !== activeChannelId) return
        if (data.unpin) {
          setMessages(prev => prev.map(m =>
      ...
      chatWs.on('chat.system', (data: { channel_id?: string; notification?: { text?: string } }) => {
        if (data.channel_id !== activeChannelId) return
  ...
  const togglePin = useCallback((messageId: string) => {
    ...
    setPinnedMessages(prev => {
      ...
      if (m) return [...prev.filter(x => x.message_id !== messageId), { ...m, pinned_at: new Date().toISOString() }]
      return prev
    })
  ...
  const toggleReaction = useCallback((messageId: string, emoji: string) => {
    ...
    setMessages(prev => prev.map(m => {
      ...
      if (remove) { const idx = users.indexOf(userId); if (idx >= 0) users.splice(idx, 1) }
  ...
  const handleSetActiveChannel = useCallback((channelId: string | null) => {
    ...
    if (channelId) setUnreadCounts(prev => { const next = { ...prev }; delete next[channelId]; return next })
  }, [])
